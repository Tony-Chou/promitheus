...
class PromitheusServiceInstancePolicy(TenantWithContainerPolicy):
    model_name = "PromitheusServiceInstance"
    ...
    def handle_delete(self, service_instance):
        if service_instance.instance and (not service_instance.instance.deleted):
            all_service_instances_this_instance = PromitheusServiceInstance.objects.filter(instance_id=service_instance.instance.id)
            ...
            if (not other_service_instances_this_instance):
                self.logger.info("Promitheus Instance %s is now unused -- deleting" % service_instance.instance)
                service_instance.instance.delete()
            else:
                self.logger.info("Promitheus Instance %s has %d other service instances attached" % (service_instance.instance, len(other_service_instances_this_instance)))
